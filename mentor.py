...
def _sys_msg(static_system_prompt: str) -> dict:
    ...
    - MENTOR_USE_CACHE=false (env) → ปิด cache_control สนิท ไว้ทดสอบเทียบ token/cost มี-ไม่มี cache
    """
    if not MENTOR_USE_CACHE:
        return {"role": "system", "content": static_system_prompt}
    return {
        "role": "system",
        "content": [{
            "type": "text",
